...
/**
 * @desc    Get Global System Defaults (Proctoring & Exam)
 * @route   GET /api/admin/config/defaults
 * @access  Private (Admin)
 */
export const getSystemDefaults = async (req: Request, res: Response) => {
    try {
        const config = await (SystemConfig as any).getOrCreate();
        res.json({
            proctoringDefaults: config.proctoringDefaults,
            examDefaults: config.examDefaults
        });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
...
/**
 * @desc    Update Global System Defaults
 * @route   PUT /api/admin/config/defaults
 * @access  Private (Admin)
 */
export const updateSystemDefaults = async (req: Request, res: Response) => {
    try {
        const schema = z.object({
            proctoringDefaults: z.object({
                ...
                violationThreshold: z.number().min(1).max(100).optional(),
                performanceSettings: z.object({
                    gazeYawThreshold: z.number().min(0).max(180).optional(),
                    faceScoreThreshold: z.number().min(0).max(1).optional(),
                    audioRMSThreshold: z.number().min(0).max(1).optional(),
                    violationCooldownMs: z.number().min(0).optional()
                }).optional()
            }).optional(),
        ...
        if (data.proctoringDefaults) {
            config.proctoringDefaults = { ...config.proctoringDefaults, ...data.proctoringDefaults };
        }
        if (data.examDefaults) {
            config.examDefaults = { ...config.examDefaults, ...data.examDefaults };
        }
